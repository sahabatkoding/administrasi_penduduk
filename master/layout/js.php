<script src="<?= $ROOT ?>assets/vendors/scripts/core.js"></script>
<script src="<?= $ROOT ?>assets/vendors/scripts/script.min.js"></script>
<script src="<?= $ROOT ?>assets/vendors/scripts/process.js"></script>
<script src="<?= $ROOT ?>assets/vendors/scripts/layout-settings.js"></script>
<!-- <script src="<?= $ROOT ?>assets/src/plugins/apexcharts/apexcharts.min.js"></script> -->
<script src="<?= $ROOT ?>assets/src/plugins/datatables/js/jquery.dataTables.min.js"></script>
<script src="<?= $ROOT ?>assets/src/plugins/datatables/js/dataTables.bootstrap4.min.js"></script>
<script src="<?= $ROOT ?>assets/src/plugins/datatables/js/dataTables.responsive.min.js"></script>
<script src="<?= $ROOT ?>assets/src/plugins/datatables/js/responsive.bootstrap4.min.js"></script>
<!-- <script src="<?= $ROOT ?>assets/vendors/scripts/dashboard.js"></script> -->
<!-- buttons for Export datatable -->
<script src="<?= $ROOT ?>assets/src/plugins/datatables/js/dataTables.buttons.min.js"></script>
<script src="<?= $ROOT ?>assets/src/plugins/datatables/js/buttons.bootstrap4.min.js"></script>
<script src="<?= $ROOT ?>assets/src/plugins/datatables/js/buttons.print.min.js"></script>
<script src="<?= $ROOT ?>assets/src/plugins/datatables/js/buttons.html5.min.js"></script>
<script src="<?= $ROOT ?>assets/src/plugins/datatables/js/buttons.flash.min.js"></script>
<script src="<?= $ROOT ?>assets/src/plugins/datatables/js/pdfmake.min.js"></script>
<script src="<?= $ROOT ?>assets/src/plugins/datatables/js/vfs_fonts.js"></script>
	<!-- Datatable Setting js -->
<script src="<?= $ROOT ?>assets/vendors/scripts/datatable-setting.js"></script></body>
<script src="../<?php echo $ROOT ?>assets/library/function.js"></script>

<script type="text/javascript" src="../<?php echo $ROOT ?>assets/src/plugins/jquery-validation/dist/jquery.validate.js"></script>
<script type="text/javascript" src="../<?php echo $ROOT ?>assets/src/plugins/jquery-validation/dist/jquery.validate.min.js"></script>

<script type="text/javascript" src="../<?php echo $ROOT ?>assets\src\plugins\md5\js\md5.min.js"></script>
<script type="text/javascript" src="../<?php echo $ROOT ?>assets\src\plugins\md5\js\md5.js"></script>
<!-- <script type="text/javascript" src="../<?php echo $ROOT ?>assets\src\plugins\select2\dist\js\select2.js"></script>
<script type="text/javascript" src="../<?php echo $ROOT ?>assets\src\plugins\select2\dist\js\select2.min.js"></script> -->
<!-- <script type="text/javascript" src="<?php echo $ROOT ?>assets/src/plugins/bootstrap-select/dist/"></script> -->
<!-- <script type="text/javascript" src="../<?php echo $ROOT ?>assets\src\plugins\select2\dist\js\select2.full.js"></script>
<script type="text/javascript" src="../<?php echo $ROOT ?>assets\src\plugins\select2\dist\js\select2.full.min.js"></script> -->